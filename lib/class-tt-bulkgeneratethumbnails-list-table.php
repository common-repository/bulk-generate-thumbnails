 ...
 * @subpackage BulkGenerateThumbnails List Table
 ...
 * List table
 */
class TT_BulkGenerateThumbnails_List_Table extends WP_List_Table {
	...
	private function read_data() {
		...
		$search_text = get_user_option( 'bgth_search_text', get_current_user_id() );
		if ( isset( $_POST['bulk-generate-thumbnails-filter'] ) && ! empty( $_POST['bulk-generate-thumbnails-filter'] ) ) {
			if ( check_admin_referer( 'bgth_filter', 'bulk_generate_thumbnails_filter' ) ) {
				if ( ! empty( $_POST['user_id'] ) ) {
					update_user_option( get_current_user_id(), 'bgth_filter_user', absint( $_POST['user_id'] ) );
				} else {
				...
				if ( ! empty( $_POST['monthly'] ) ) {
					...
					update_user_option( get_current_user_id(), 'bgth_filter_monthly', $monthly_arr );
				} else {
				...
				if ( ! empty( $_POST['search_text'] ) ) {
					...
					update_user_option( get_current_user_id(), 'bgth_search_text', $search_text );
				} else {
					delete_user_option( get_current_user_id(), 'bgth_search_text' );
					$search_text = null;
				}
			}
		}
		...
		if ( current_user_can( 'manage_options' ) ) {
			$author = get_user_option( 'bgth_filter_user', get_current_user_id() );
		} else {
		...
		$args = array(
			...
			'post_status'    => 'any',
			'author'         => $author,
			'post_mime_type' => get_user_option( 'bgth_filter_mime_type', get_current_user_id() ),
			'posts_per_page' => -1,
			'orderby'        => 'date',
			'order'          => 'DESC',
		);
		...
		$monthly_archive = get_user_option( 'bgth_filter_monthly', get_current_user_id() );
		...
		if ( ! empty( $posts ) ) {
			...
			$handle = 'selectgeneratethumbnails-js';
			$action1 = 'bulkgeneratethumbnails-ajax-action';
			$action2 = 'bulkgeneratethumbnails_message';
			wp_enqueue_script( $handle, plugin_dir_url( __DIR__ ) . 'js/jquery.selectgeneratethumbnails.js', array( 'jquery' ), '1.00', false );
	...
	public function column_title( $item ) {
		...
		return sprintf(
			...
			/*$1%s*/ $view_thumb_url . $filename_html . $id_html
		);
	}
	...
	public function column_cb( $item ) {
		...
		return sprintf(
			'<input type="checkbox" name="%1$s[%2$s]" value="%3$s" form="selectgeneratethumbnails_forms" />',
	...
	public function print_column_headers( $with_id = true ) {
		...
		if ( isset( $_SERVER['HTTP_HOST'] ) && ! empty( $_SERVER['HTTP_HOST'] ) &&
			...
			/* Customize for nonce */
			$current_url = wp_nonce_url( $current_url, 'bgth_sort_nonce' );
		} else {
			wp_die();
		}
		...
		if ( wp_verify_nonce( $nonce, 'bgth_sort_nonce' ) ) {
	...
	public function prepare_items() {
		 ...
		 */
		$per_page = get_user_option( 'bgth_per_page', get_current_user_id() );
		 ...
		 */
		$data = $this->read_data();
		do_action( 'bgth_filter_form', get_current_user_id() );
